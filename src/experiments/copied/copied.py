...
class Agent:
    ...
    def MAXQ_0(self, i, s):  # i is action number
        ...
        if self.is_primitive(i):
            self.new_s, r, self.done, truncated, _ = copy.copy(self.env.step(i))
            if self.fitted:
                self.env.render()
                time.sleep(0.5)
...
for j in range(episodes):
    if j == 5000:
        print('fitted')
    taxi.MAXQ_0(10, state)  # start in root
...
taxi.reset()
taxi.env = gym.make('Taxi-v3', render_mode="human")
state, info = taxi.env.reset()
taxi.fitted = True
taxi.MAXQ_0(10, state)
