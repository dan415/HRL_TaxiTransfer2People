...
class HRLAgent2P:
    ...
    def __init__(self, env, alpha, gamma, epsilon, render=False, vtable=None, ctable=None):
        ...
        self.FETCH_CLIENT1 = 8
        self.DRIVE_CLIENT1 = 9
        self.FETCH_CLIENT2 = 10
        self.DRIVE_CLIENT2 = 11
        ...
        self.actions = [
            ...
            [self.NAVIGATE_S, self.PICKUP],  # 8: FETCH_CLIENT1
            [self.NAVIGATE_D, self.DROPOFF],  # 9: DRIVE_CLIENT1
            [self.NAVIGATE_S, self.PICKUP],  # 10: FETCH_CLIENT2
            [self.NAVIGATE_D, self.DROPOFF],  # 11: DRIVE_CLIENT2
    ...
    def is_terminal(self, a, done):
        ...
        if a == self.ROOT:
            ...
            passidx = pass_loc1 if self.passenger == 1 else pass_loc2
            result = not self.passenger or (passidx != 4 and taxiloc == self.RGBY[passidx])
        elif a == self.NAVIGATE_D:
            passidx = pass_loc1 if self.passenger == 1 else pass_loc2
            destidx = dest_idx1 if self.passenger == 1 else dest_idx2
            result = not self.passenger or (passidx == 4 and taxiloc == self.RGBY[destidx])
